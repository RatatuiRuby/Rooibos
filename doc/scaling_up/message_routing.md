...
Leaves handle the `:increment` envelope with a plain Update lambda. No Router needed:
...
module Leaf
  ...
  Update = ->(message, model) {
    return unless message.routed? && message.envelope == :increment
...
module Panel
  ...
  # Receive increment for self when routed message says :panel_self
  receive_routed :panel_self,
    -> (_, model) {
      count = model.count + 1
      case count
  ...
  # Root sends :leaf_1, :leaf_2 (our API). Transform to :increment for leaves
...
Root sends semantic envelopes in Panel's API. It knows `:panel_self`, `:leaf_1`, and `:leaf_2`. It doesn't know Panel's internal route names:
...
module Root
  ...
  # Both routes use the same fragment (Panel), so you must use the model attribute symbol 
  # to target them. `to: Panel` would be ambiguous.
  ...
  # Semantic envelopes in Panel's API: :panel_self, :leaf_1, :leaf_2
  # Root doesn't know Panel's internal structure (top_leaf, bottom_leaf)
...
```
Root       →  Panel      →  Leaf
:leaf_1    →  :increment →  (handles)
```

Each layer speaks its inner fragment's API. Root sends `:leaf_1`. Panel transforms to `:increment` via `as:`. Leaf handles it.

**Inward flow:**
- `Enter` → Root's receive matches → Root handles its own counter
- `a` → Root forwards `:panel_self` to `:left_panel` → Panel's receive matches → Panel handles its own counter
- `1` → Root forwards `:leaf_1` to `:left_panel` → Panel transforms to `:increment` → Leaf handles it

**Outward flow (bubble):**
Leaf reaches 10 → bubbles `LeafReset` → Panel's `observe` runs → Root's `observe` runs

**Outward flow (deliver):**
Leaf reaches 5 → delivers `Milestone` directly to Root → Panel never sees it

Bubble flows through each fragment. Deliver skips to Root.

**Encapsulation:**
If Panel renames `top_leaf` to `primary`, only Panel's forward declarations change. Root's keybindings stay the same. Root knows Panel's API, not its internals.
