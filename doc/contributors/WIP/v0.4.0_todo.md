...
## 4. ~~Update Doc Comments to Show Hash Pattern Matching~~ ✅ DONE

---

## 5. ~~Update widget_command_system Example~~ ✅ DONE

---

## 6. ~~Rooibos.delegate Array Wrapping~~ ✅ DONE

---

## 7. ~~Missing Tests (Technical Debt)~~ ✅ DONE

---

## 8. ~~Update test_widget_command_system.rb~~ ✅ DONE
...
## 9. Add `out.standing` for Parallel Streaming Commands
...
Add `out.standing` to stand up async child commands that stream messages live. These remain
standing until they complete or are cancelled. Many outstanding commands will live for the
remainder of the application. But when you expect them to complete, you can outwait them
to block on their completion.

> [**standing**](https://en.wiktionary.org/wiki/standing): adjective. Not transitory; not liable to fade or vanish; lasting.

> [**outstanding**](https://en.wiktionary.org/wiki/outstanding): adjective. Unresolved; not settled or finished. 

> [**wait**](https://en.wiktionary.org/wiki/wait): verb. To delay movement or action until the arrival or occurrence of; to await.

> [**outwait**](https://en.wiktionary.org/wiki/outwait): verb. To wait for something to end.


```ruby
# API:
handle = out.standing(command, token)              # messages pass directly to runtime
handle = out.standing(command, token, processor)   # messages go through processor first
out.wait(handle1, handle2, ...)                    # block until all handles complete
```

This not-just-wordplay-but-specifically-what-it-does wording works well with `Out#put`.

> [**put**](https://en.wiktionary.org/wiki/put): verb. To physically place (something or someone somewhere).

> [**output**](https://en.wiktionary.org/wiki/output): noun. Data sent from one program on the computer to another.
...
class LoadDashboard < Data.define(:user_id, :tag)
  ...
  def call(out, token)
    ...
    h1 = out.standing(StreamNotifications.new(auth[:token]), token)
...
#### 2. Add `standing` to `Outlet`

```ruby
def standing(command, token, processor = nil)
...
```ruby
def test_standing_streams_messages_live
  # Custom command that:
  # 1. source(single_command) - sync
  # 2. standing(streaming_cmd1), standing(streaming_cmd2)
  # 3. wait(h1, h2)
  # Verify messages arrive interleaved as streams produce them
end

def test_standing_with_processor_transforms_messages
  # Verify processor.call is invoked for each message
  # Verify transformed output reaches update function
end

def test_standing_crash_produces_command_error
  # Verify child crash doesn't hang — becomes Command::Error
end

def test_standing_processor_crash_produces_command_error
