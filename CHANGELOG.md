...
- `Command.bubble` from a nested fragment now correctly propagates through intermediate fragments that have no `observe` or `intercept` for that bubble. Previously, bubbles were silently dropped when the immediate outer fragment had no outward handler, even if a further-out fragment did.
